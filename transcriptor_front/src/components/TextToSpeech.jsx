...
export const TextToSpeech = () => {
  ...
  const handlGenerarAudio = async () => {
    ...
    setVisible(true)
    try {
      const response = await fetch('http://5.5.4.39:5001/synthesize', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex flex-col items-center  min-h-[70vh]">
      <h2 className="text-2xl font-semibold pt-5">CONVERSOR TEXTO A VOZ</h2>
      <section id="textToSpeechSection">
        ...
        <div id="languageSelection">
          <label htmlFor="selectLanguage" id="labelLanguage">
          ...
          <button id="btnCrearAudio" onClick={handlGenerarAudio} disabled={visible ? true : false}>
            {visible?"Creando audio...":"Crear"}
          </button>
        </div>
        <div id="audioControls" className="w-full flex flex-col items-center justify-center" style={audioPlay === "" ? {display:"none"}: {display:"block"}} >
          <h3>Audio Generado:</h3>

          <audio id="audioPlayer" controls src={audioPlay === "" ? null : audioPlay}></audio>
        </div>
      </section>
    </div>
  );
